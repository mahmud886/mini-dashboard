...
import { ChevronLeft, ChevronRight, Home, Newspaper, Users } from 'lucide-react';
      ...
      >
        <div className="flex h-full flex-col">
          <div className="flex h-14 items-center justify-between px-3">
            <Link href="/" className={cn('text-sm font-semibold', (isDesktop ? collapsed : !drawerOpen) && 'sr-only')}>
              Mini Dashboard
            </Link>
          </div>
          ...
          {/* Static footer (design-only) */}
          <div className="mt-auto border-t border-zinc-700 p-2">
            <div className="px-2 py-2 text-xs text-zinc-500">© {new Date().getFullYear()} Mini Dashboard</div>
          </div>
        </div>
      </motion.aside>
